...
spark = SparkSession.builder.appName("HappinessTrendAnalysisForFinland").getOrCreate()

# List of years for which you have data
years = [2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021, 2023]  # Extend this list according to the data you have
...
for year in years:
    ...
    # Check if the DataFrame is empty and continue to the next iteration if so
    if finland_df.head(1):
        ...
        # Optionally log or handle years with no data for Finland
        print(f"No data for Finland in {year}")

# Once all data is combined, you can save it to a CSV for visualization
output_path = f'{base_directory}finland_happiness_scores.csv'
if finland_data_df:
    finland_data_df.write.csv(output_path, header=True, mode="overwrite")
else:
    print("No data available for Finland in the specified years.")
