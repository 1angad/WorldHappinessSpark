...
spark = SparkSession.builder.appName("HighestScoresCorrelation").getOrCreate()
...
highest_scores_df = df.filter(df['Score Type'] == 'Highest')
...
highest_scores_df = highest_scores_df.dropna(subset=columns_of_interest)
...
df_vector = vectorAssembler.transform(highest_scores_df)
...
correlation_df.to_csv('/home/Muhammed/highest_scores_correlation_matrix.csv')
